...
DESCRIPTION:	This program receives a 4 KB message (1024 ints), reverses the 
				...
				work with sender.cpp. It also works with trigger.cpp in 
				receiving a proxy.
...
if (argc != 2)
	{
	printf("Incorect command line: receiver receiverName\n");
	exit(-1);
	}
...
while (true)
	...
	cout << "sender's reply buffer size=" << nee.getSenderRplySize(senderId) << endl;
